...
// 初始化玻璃卡片的Lottie动画
function initGlassCardLottie() {
  const container = document.getElementById('lottie-container');
  if (container && typeof lottie !== 'undefined') {
    try {
      console.log('开始初始化玻璃卡片Lottie动画...');
      
      glassCardLottieAnimation = lottie.loadAnimation({
        container: container,
        renderer: 'svg',
        loop: true,
        autoplay: false,
      ...
      glassCardLottieAnimation.addEventListener('complete', () => {
        console.log('玻璃卡片Lottie动画播放完成');
        updateAnimationStatus('播放完成');
      });
      
      glassCardLottieAnimation.addEventListener('loopComplete', () => {
        console.log('玻璃卡片Lottie动画循环完成');
        updateAnimationStatus('循环播放中');
      });
      
      glassCardLottieAnimation.addEventListener('data_ready', () => {
        console.log('玻璃卡片Lottie动画数据加载完成');
        updateAnimationStatus('已加载，等待悬浮');
      });
      
      glassCardLottieAnimation.addEventListener('data_failed', () => {
        console.error('玻璃卡片Lottie动画数据加载失败');
        updateAnimationStatus('加载失败');
      });
      
      console.log('玻璃卡片Lottie动画初始化完成');
      
    } catch (error) {
      console.error('玻璃卡片Lottie动画初始化错误:', error);
      updateAnimationStatus('初始化失败');
    }
  } else {
    console.error('Lottie容器未找到或lottie库未加载');
    updateAnimationStatus('容器未找到');
  }
}
...
function updateCounters() {
  const hoverElement = document.getElementById('hover-count');
  const clickElement = document.getElementById('click-count');
  
  if (hoverElement) {
    hoverElement.textContent = hoverCount;
  }
  if (clickElement) {
...
function onCardHover() {
  hoverCount++;
  console.log('鼠标悬浮在玻璃卡片上，悬浮次数:', hoverCount);
  ...
  if (glassCardLottieAnimation) {
    console.log('开始播放玻璃卡片Lottie动画');
    glassCardLottieAnimation.play();
    updateAnimationStatus('播放中');
  } else {
    console.log('玻璃卡片Lottie动画未初始化');
    updateAnimationStatus('动画未初始化');
  }
}
...
function onCardClick() {
  clickCount++;
  console.log('玻璃卡片被点击，点击次数:', clickCount);
