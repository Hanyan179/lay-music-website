package com.fateechoes.service.impl;
...
public class EventServiceImpl implements EventService {
    ...
    @Override
    public List<EventDTO> getEventsByBookCode(Long bookCode) {
        List<Event> events = eventRepository.findByBookIdOrderByYearAsc(bookCode);
    ...
    @Override
    public Map<String, PersonalEventVO> buildPersonalEventsMap(Long bookCode) {
        List<Event> events = eventRepository.findByBookIdOrderByYearAsc(bookCode);
    ...
    @Override
    public String getStartingEventId(Long bookCode) {
        List<PersonalChoiceEvent> entryEvents = personalChoiceEventRepository.findByBookIdAndYearOrderByDisplayOrderAsc(bookCode,2025);
        if (!entryEvents.isEmpty()) {
            return entryEvents.get(0).getQuestion();
        }

        // 如果没有标记为起始事件的，返回第一个事件
        List<Event> allEvents = eventRepository.findByBookIdOrderByYearAsc(bookCode);
